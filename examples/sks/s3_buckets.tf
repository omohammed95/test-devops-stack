...
# resource "exoscale_iam_api_key" "s3_iam_key" {
#   for_each = toset(local.s3_buckets)
...
resource "exoscale_iam_role" "omo_role" {
  name        = "omo-role"
  description = "My role for SOS access"
  editable    = true
...
resource "exoscale_iam_api_key" "s3_iam_key" {
  for_each = toset(local.s3_buckets)
  name     = "${local.cluster_name}-${each.key}-s3-iam-key"
  role_id  = exoscale_iam_role.omo_role.id

  depends_on = [
    aws_s3_bucket.this,
    exoscale_iam_role.omo_role,
  ]
}
